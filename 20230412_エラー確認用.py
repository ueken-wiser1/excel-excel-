...
#testのため、仮フォルダ設定
dirmerge = "C:/Users/touko/OneDrive/株価分析/excel/株式データ/test/20230412/"
...
#移動平均数値の取込
        if lastrow_stockbook > 7:
                ...
                if lastrow_stockbook > 11:
                        stockprice_9dEmovemean = bn.move_mean(stockprice, window=9)
...
#        print('移動平均', bn.move_mean(stockprice, window = 5))
        for j in range(2, lastrow_stockbook):
                if lastrow_stockbook > 7:
                        ...
                        if lastrow_stockbook > 11:
                                sheetstock.cell(row=j,column=320).value = stockprice_9dEmovemean[j-2] + (2/10)*(sheetstock.cell(j,4).value - stockprice_9dEmovemean[j-2])
                                if lastrow_stockbook > 12:
