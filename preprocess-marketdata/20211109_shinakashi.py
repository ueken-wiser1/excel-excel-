...
dir_data = "C:/Users/touko/OneDrive/株価分析/excel/株式データ/ダウンロードデータ/05.品貸料率/"
...
#    print(row[2])
    code_databook = str(row[2])
    rentStockOver = str(row[8])
    maxRatio = str(row[9])
    todayRatio = str(row[10])
    yesterdayRatio = str(row[12])
...
#                print(maxRatio)
#                print(market_list[0])
                sheet02.cell(row=i,column=121).value = rentStockOver
                sheet02.cell(row=i,column=122).value = maxRatio
                sheet02.cell(row=i,column=123).value = todayRatio
                sheet02.cell(row=i,column=124).value = yesterdayRatio
...
databook.close()
new_path = shutil.move(data_list[0],"C:/Users/touko/OneDrive/株価分析/excel/株式データ/ダウンロードデータ/05.品貸料率/完了/")
