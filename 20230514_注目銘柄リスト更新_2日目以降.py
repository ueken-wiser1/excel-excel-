...
for watch_list_file in glob.glob(os.path.join(watch_list_folder, '*.xlsx')):
    file_name = os.path.basename(watch_list_file)
...
#date_rowを取得してるから、そこからlastrowまで引っ張る形にするか。
            if date_row is None:
                continue
            k=last_col_watch
            for i in range(date_row, date_row+10):
                #print("daterow" + str(date_row))
    ...
    #            continue
                ...
                stock_date = ws_stock_data.cell(row=i,column=1).value
                ws_watch_list.cell(row=1, column=k).value = stock_date
                closing_price = ws_stock_data.cell(row=i, column=4).value
                ws_watch_list.cell(row=row, column=k).value = closing_price
                
                #print("証券コード"+str(stock_code) +"は、終値"+str(ws_watch_list.cell(row=row, column=k).value)+"を示しました")
                start_price = ws_watch_list.cell(row=row, column=29).value
                #print(k)
                #print(ws_watch_list.cell(row=row, column=k).value)
                #print(ws_watch_list.cell(row=row, column=k-1).value)
                if ws_watch_list.cell(row=row, column=k).value is None:
                    print("証券コード："+str(stock_code)+"は上場廃止になったかも。")
                    continue
                diff=ws_watch_list.cell(row=row, column=k).value - ws_watch_list.cell(row=row, column=k-1).value
                if diff > 0:
                    ws_watch_list.cell(row=row, column=k).fill = plus_fill
                elif diff < 0:
                    ws_watch_list.cell(row=row,column=k).fill = minus_fill
                ...
                if closing_price is not None and start_price is not None:
                    ...
                    if ratio > 2:
                        ...
                        for j in range(1, n_columns):
                            ws_watch_list.cell(row=row, column=j).fill =attained_fill

                        profit = ws_watch_list.cell(row=row,column=k).value - ws_watch_list.cell(row=row,column=29).value
                        ...
                        for m in range(1, n_columns):
                            ws_watch_list.cell(row=row, column=m).fill =unattained_fill
                        loss = ws_watch_list.cell(row=row,column=k).value - ws_watch_list.cell(row=row,column=29).value
...
#                        print(str(ws_watch_list.cell(row=row,column=4).value)+"_"+str(ws_watch_list.cell(row=row,column=5).value) + "は目標を達成できず、5日期限を迎えました。損益は"+str(loss)+"円です。")
                ...
                basis_date2 = basis_date + datetime.timedelta(days=1)
                    # 指定日が平日になるまでループ
                while basis_date2.weekday() == 0 or basis_date2.weekday() == 1 or basis_date2.weekday() == 2 or basis_date2.weekday() == 3 or basis_date2.weekday() == 4 and not jpholiday.is_holiday(basis_date2) is True:
                    # 指定日が土曜日または日曜日の場合
    ...
    if last_col_last >= 42:
        print(str(watch_list_file)+"は調査期間を満了しました。")
        if os.path.exists(dirstorage+file_name) is True:
            os.remove(watch_list_file)
            
    # 移動先のパスが既に存在する場合の処理
    # 例えば、別の名前で保存するなどの対処方法を選択する
    # または、ファイルを上書きする場合は、os.remove() を使用して既存のファイルを削除する
    # 例: os.remove(dirstorage)
        else:
