...
for l in stock_list:
    ...
    print(sheet01.cell(row=2,column=3).value)
...
#連騰回数
            ...
            if k<10:
                if scoresheet_allscore.cell(row=i,column=4).value < 500:
                    scoresheet_upsort.cell(row=j+12,column=yokozure*5+2).value = df_s.iat[i-2,1]
                    scoresheet_upsort.cell(row=j+12,column=yokozure*5+3).value = df_s.iat[i-2,2]
                    scoresheet_upsort.cell(row=j+12,column=yokozure*5+4).value = df_s.iat[i-2,3]
                    scoresheet_upsort.cell(row=j+12,column=yokozure*5+5).value = df_s.iat[i-2,33]
                    k += 1
                else:
                    pass
            else:
        ...
        for i in range(2,lastrow_scorebook+1):
            ...
            if k<10:
                if scoresheet_allscore.cell(row=i,column=4).value < 500:
                    scoresheet_downsort.cell(row=j+12,column=yokozure*5+2).value = df_s.iat[i-2,1]
                    scoresheet_downsort.cell(row=j+12,column=yokozure*5+3).value = df_s.iat[i-2,2]
                    scoresheet_downsort.cell(row=j+12,column=yokozure*5+4).value = df_s.iat[i-2,3]
                    scoresheet_downsort.cell(row=j+12,column=yokozure*5+5).value = df_s.iat[i-2,33]
                    k += 1
                else:
                    pass
            else:
...
sort_and_paste('標準化出来高変化率',True,1)
sort_and_paste('標準化出来高変化率',False,1)
...
sort_and_paste('標準化信用買残変化率',True,4)
sort_and_paste('標準化信用買残変化率',False,4)
