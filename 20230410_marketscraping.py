...
#           利回り
    try:
        rimawari = str(soup.select('td')[20])
    except IndexError as e:
        print('利回り存在しない')
        write_column = 61
        pass
        ...
        write_column = 61
        sheet.cell(row=j, column=write_column).value = rimawari
        write_column += 1

#           売上高
    try:
        uriage = str(soup2.select('td')[239])
    except IndexError as e:
        print('売上高存在しない')
        write_column = 62
        pass
        ...
        write_column = 62
        sheet.cell(row=j, column=write_column).value = uriage
        write_column += 1

#           営業益
    try:
        eigyoeki = str(soup2.select('td')[240])
    except IndexError as e:
        print('営業益存在しない')
        write_column = 63
        pass
        ...
        write_column = 63
        sheet.cell(row=j, column=write_column).value = eigyoeki
        write_column += 1

#           経常益
    try:
        keijoeki = str(soup2.select('td')[54])
    except IndexError as e:
        print('営業益存在しない')
        write_column = 64
        pass
        ...
        write_column = 64
        sheet.cell(row=j, column=write_column).value = keijoeki
        write_column += 1


#           最終益
    try:
        saishueki = str(soup2.select('td')[55])
    except IndexError as e:
        print('売上営業利益率存在しない')
        write_column = 65
        pass
        ...
        write_column = 65
        sheet.cell(row=j, column=write_column).value = saishueki
        write_column += 1

#           売上営業利益率
    try:
        riekiritsu = str(soup2.select('td')[241])
    except IndexError as e:
        print('売上営業利益率存在しない')
        write_column = 66
        pass
        ...
        write_column = 66
        sheet.cell(row=j, column=write_column).value = riekiritsu
        write_column += 1

#           ROE
    try:
        ROE = str(soup2.select('td')[242])
    except IndexError as e:
        print('ROE存在しない')
        write_column = 67
        pass
        ...
        write_column = 67
        sheet.cell(row=j, column=write_column).value = ROE
        write_column += 1

#           ROA
    try:
        ROA = str(soup2.select('td')[243])
    except IndexError as e:
        print('ROA存在しない')
        write_column = 68
        pass
        ...
        write_column = 68
        sheet.cell(row=j, column=write_column).value = ROA
        write_column += 1

#           総資産回転率
    try:
        kaitenritsu = str(soup2.select('td')[244])
    except IndexError as e:
        print('総資産回転率存在しない')
        write_column = 69
        pass
        ...
        write_column = 69
        sheet.cell(row=j, column=write_column).value = kaitenritsu
        write_column += 1

#           フリーキャッシュフロー
    try:
        freeCF = str(soup2.select('td')[308])
    except IndexError as e:
        print('フリーキャッシュフロー存在しない')
        write_column = 70
        pass
    else:
        write_column = 70
        sheet.cell(row=j, column=write_column).value = freeCF
        write_column += 1

#           営業キャッシュフロー
    try:
        eigyoCF = str(soup2.select('td')[309])
    except IndexError as e:
        print('営業キャッシュフロー存在しない')
        write_column = 71
        pass
    else:
        write_column = 71
        sheet.cell(row=j, column=write_column).value = eigyoCF
        write_column += 1

#           投資キャッシュフロー
    try:
        toushiCF = str(soup2.select('td')[310])
    except IndexError as e:
        print('投資キャッシュフロー存在しない')
        write_column = 72
        pass
    else:
        write_column = 72
        sheet.cell(row=j, column=write_column).value = toushiCF
        write_column += 1

#           財務キャッシュフロー
    try:
        zaimuCF = str(soup2.select('td')[311])
    except IndexError as e:
        print('財務キャッシュフロー存在しない')
        write_column = 73
        pass
    else:
        write_column = 73
        sheet.cell(row=j, column=write_column).value = zaimuCF
        write_column += 1

#           現金
    try:
        genkin = str(soup2.select('td')[312])
    except IndexError as e:
        print('現金存在しない')
        write_column = 74
        pass
        ...
        write_column = 74
        sheet.cell(row=j, column=write_column).value = genkin
        write_column += 1

#           1株益
    try:
        onekabueki = str(soup2.select('td')[245])
    except IndexError as e:
        print('1株益存在しない')
        write_column = 75
        pass
        ...
        write_column = 75
        sheet.cell(row=j, column=write_column).value = onekabueki
        write_column += 1

    if sheet.cell(j,3).value == '株探からのお知らせ':
        sheet.delete_rows(j)
        print(sheet.cell(j,2).value + '削除')
    elif sheet.cell(j,4).value == '株価':
        sheet.delete_rows(j)
        print(sheet.cell(j,2).value + '削除')
    else:
        pass
